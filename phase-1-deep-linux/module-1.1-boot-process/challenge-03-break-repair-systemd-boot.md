...
The first screenshot records the Arch live ISO, `lsblk -f`, the root and ESP mounts, and the files found in the installed system:
...
The second screenshot records the broken entry, the missing configured kernel path, the intact real kernel, and the comparison with the known-good backup:
...
The third screenshot records the chroot repair, validation against the backup, and clean unmount. It also shows the harmless initial `/mount` typo before the correct `umount -R /mnt` command:
...
Before repairing it, I proved that the configured path was missing while the real kernel still existed and compared the broken entry with the known-good backup:
...
diff -u /mnt/root/arch.conf.known-good \
  /mnt/boot/loader/entries/arch.conf
```

I then entered the installed system and corrected the entry from inside the chroot:
...
The file repaired inside the chroot was:
...
Before leaving the chroot, I verified the entry, confirmed that the real kernel was present, and compared the repaired entry with the backup:

```bash
grep -nE '^(title|linux|initrd|options)' /boot/loader/entries/arch.conf
ls -l /boot/vmlinuz-linux
diff -u /root/arch.conf.known-good /boot/loader/entries/arch.conf
```

I then exited the chroot and unmounted the filesystems:
...
`findmnt -R /mnt` returned no output, confirming that the recovered installation was fully unmounted. After powering off and removing the live ISO, the VM booted from the virtual disk normally.
